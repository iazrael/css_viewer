...
function CSSViewer(uid){
    ...
    var viewer = {
        init: function(){
            ...
            this.onDocumentMouseOver = function(e){
                ...
                chrome.extension.sendRequest({type: 'event', param: {type: "ViewerShow", uid: uid}, tabId: tabId});
            }
    ...
    var styleSheet = {
        ...
        load: function(){
            ...
            if(isEmptyObject(sheets)){
                ...
                chrome.extension.sendRequest({type: 'method', param: {method: "translateSheets", data: sheets, uid: uid}, tabId: tabId});
            }
        },
    ...
    var handleEvent = function(param){
        switch(param.type){
            case 'ViewerShow':
                if(param.uid !== uid){
                    viewer.hide();
    ...
    chrome.extension.onRequest.addListener(function(request, sender, callback){
        ...
        if(request.type === 'method'){
            ...
            result = handleEvent(request.param);
        }
        result = result || {};
        result.tabId = tabId;
        callback(result);
    ...
    if(isIframe()){
        chrome.extension.sendRequest({type: 'method', param: {method: "queryRunStatus"}, tabId: tabId}, function(data){
            if(parseInt(data) === STATUS.RUNNING){
                viewer.start();
            }
        });
    }
...
chrome.extension.sendRequest({type: 'method', param: {method: "contentReady", isIframe: isIframe()}, tabId: tabId}, function(data){
    ...
    if(isIframe()){
        chrome.extension.sendRequest({type: 'method', param: {method: "getOption", key: 'enableIframe'}, tabId: tabId}, function(data){
            var enableIframe = parseInt(data);
            if(enableIframe === 1){
                new CSSViewer(uid);
            }
        });
